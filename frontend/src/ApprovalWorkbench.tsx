...
type Lease = { proposalRevisionId: string; leaseToken: string; leaseVersion: number; expiresAt: string };
...
export function ApprovalWorkbench({ approverId }: { approverId: string }) {
  ...
  async function claim(revisionId: string) {
    ...
    try {
      const response = await fetch(`/api/approver/compensation-proposals/${revisionId}/claims`, {
        ...
        headers: { ...headers, "Content-Type": "application/json", "Idempotency-Key": globalThis.crypto.randomUUID() },
  ...
  async function consumeEvents(lease: Lease, cursor: string) {
    ...
    try {
      const response = await fetch(`/api/approver/compensation-proposals/${lease.proposalRevisionId}/approval-view/events`, {
  ...
  async function decide(decision: "approve" | "reject") {
    ...
    const body = decision === "approve"
      ? { proposalRevision: snapshot.proposalRevision, contentDigest: snapshot.contentDigest, internalNote: "符合当前审批证据" }
      : { proposalRevision: snapshot.proposalRevision, contentDigest: snapshot.contentDigest, internalReason: "需要人工复核" };
    try {
      const response = await fetch(`/api/approver/compensation-proposals/${lease.proposalRevisionId}/${decision}`, {
        method: "POST", credentials: "same-origin",
        headers: { ...leaseHeaders(lease), "Content-Type": "application/json", "Idempotency-Key": globalThis.crypto.randomUUID() },
  ...
  return (
    <main className="support-workbench" aria-label="审批工作台">
      <header className="workbench-header">
        <div><p className="eyebrow">APPROVAL VIEW</p><h1>{snapshot ? "补偿提案审批" : "待审批补偿"}</h1></div>
      ...
      {snapshot ? (
        <section className="queue-panel" aria-label="当前审批视图">
          ...
          <h3>审批证据引用</h3>
          <ul>{snapshot.evidenceReferences.map((reference) => <li key={reference}>{reference}</li>)}</ul>
          <div className="ticket-actions">
            <button type="button" onClick={() => void decide("approve")}>批准补偿</button>
            <button type="button" onClick={() => void decide("reject")}>驳回并转人工</button>
          </div>
        </section>
      ) : (
        <section className="queue-panel">
          <h2>待审批补偿</h2>
          {queue.length ? <ul className="queue-list">{queue.map((item) => (
            <li key={item.proposalRevisionId}>
              <span>{item.compensationMethod} · ¥{item.amount}</span>
              <button type="button" onClick={() => void claim(item.proposalRevisionId)}>领取审批</button>
            </li>
          ))}</ul> : <p>当前没有待审批提案</p>}
        </section>
      )}
  ...
  function leaseHeaders(lease: Lease) {
    return { ...headers, "X-Approval-Lease-Token": lease.leaseToken, "X-Approval-Lease-Version": String(lease.leaseVersion) };
  }
}

function isQueueItem(value: unknown): value is QueueItem {
  return isRecord(value) && isUuid(value.proposalRevisionId) && typeof value.compensationMethod === "string"
    && typeof value.amount === "number" && typeof value.submittedAt === "string" && typeof value.expiresAt === "string";
}
function isLease(value: unknown): value is Lease {
  return isRecord(value) && isUuid(value.proposalRevisionId) && isUuid(value.leaseToken)
    && Number.isSafeInteger(value.leaseVersion) && Number(value.leaseVersion) > 0 && typeof value.expiresAt === "string";
}
function isApprovalSnapshot(value: unknown): value is ApprovalSnapshot {
  if (!isRecord(value) || !hasOnlyKeys(value, [
      "view", "schema", "cursor", "proposalRevisionId", "proposalRevision", "contentDigest",
      "orderReference", "reasonCode", "delayHours", "delaySeconds", "compensationMethod",
      "proposedAmount", "authoritativeAmount", "policyVersion", "policyTier", "eligibilityChecks",
      "evidenceReferences", "evidenceSnapshot", "responsibilityChain", "leaseToken", "leaseVersion",
      "leaseExpiresAt", "submittedAt", "proposalExpiresAt",
    ]) || value.view !== "APPROVAL_VIEW" || value.schema !== APPROVAL_SCHEMA
    || typeof value.cursor !== "string" || parseCursor(value.cursor) === null
    || !isUuid(value.proposalRevisionId) || !isUuid(value.leaseToken)
    || !Number.isSafeInteger(value.leaseVersion) || !Array.isArray(value.evidenceReferences)
    || !value.evidenceReferences.every((entry) => typeof entry === "string")
    || !Array.isArray(value.eligibilityChecks) || !value.eligibilityChecks.every((entry) => typeof entry === "string")
    || !Array.isArray(value.responsibilityChain) || !isRecord(value.evidenceSnapshot)) return false;
  return typeof value.orderReference === "string" && typeof value.policyVersion === "string"
...
function validEvent(event: SseEvent, lease: Lease, cursor: string) {
  ...
  try {
    ...
    if (!isRecord(envelope) || !hasOnlyKeys(envelope, ["view", "schema", "payload"])
      || envelope.view !== "APPROVAL_VIEW" || envelope.schema !== APPROVAL_SCHEMA || !isRecord(envelope.payload)
      || !hasOnlyKeys(envelope.payload, ["proposalRevisionId", "leaseVersion", "authorityState"])) return false;
    return event.type === "APPROVAL_AUTHORITY_STARTED"
      && envelope.payload.proposalRevisionId === lease.proposalRevisionId
      && envelope.payload.leaseVersion === lease.leaseVersion && envelope.payload.authorityState === "ACTIVE";
  } catch { return false; }
}

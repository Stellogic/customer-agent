...
export function SupportAssistance({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await humanSessionFetch(`${url}/context`, {
        ...
        dispatch({
          type: "authorize",
          assignment: { sessionKey, ticketId, assignmentId: value.assignmentId },
        });
        setNotice("");
      } catch {
        if (!active.signal.aborted)
          setNotice("辅助权限不可用；人工处理可继续，重新打开详情后重试。");
      }
    })();
    return () => {
      active.abort();
  ...
  }, [onClearDraft, sessionKey, ticketId, url]);
          ...
          : code === "MODEL_UNAVAILABLE"
            ...
            : response.status === 422
              ...
              : [400, 409].includes(response.status)
                ...
                : "retrieval";
      if (!signal.aborted)
        dispatch({ type: "complete", request, view: { status: "error", reason } });
      return;
    }
    const value: unknown = await response.json();
  ...
  async function start(kind: AssistanceKind) {
    ...
    try {
      if (active.signal.aborted) return;
      const response = await humanSessionFetch(`${url}/requests`, {
        method: "POST",
        signal: active.signal,
        credentials: "same-origin",
        cache: "no-store",
  ...
  return (
    ...
    >
      ...
      />
      {state.assignment && state.view.status === "loading" && canQuery && (
        <button type="button" onClick={() => void queryResult()}>
          查询辅助结果
        </button>
      )}
...
export function decodeAssistanceResponse(
  value: unknown,
  request: AssistanceRequest,
): AssistanceView {
  if (
    !isRecord(value) ||
    value.schema !== SCHEMA ||
    value.ticketId !== request.assignment.ticketId ||
    value.assignmentId !== request.assignment.assignmentId ||
    value.requestId !== request.requestId ||
    value.kind !== request.kind ||
    !isRecord(value.view)
  )
    throw new Error("invalid assistance response");
  const view = value.view;
  if (view.status === "loading" && view.kind === request.kind)
    return { status: "loading", kind: request.kind };
  if (
    view.status === "error" &&
    ["conflict", "index", "embedding", "model", "retrieval", "request", "format"].includes(
      String(view.reason),
    )
  )
    return {
      status: "error",
      reason: view.reason as Extract<AssistanceView, { status: "error" }>["reason"],
    };
  if (view.kind !== request.kind || view.requestId !== request.requestId)
    throw new Error("wrong assistance identity");
  if (
  ...
  if (
    ...
    view.citations.every(
      (item) =>
        isRecord(item) &&
        ["articleId", "version", "chunkId", "title", "updatedAt", "snippet"].every(
          (key) => typeof item[key] === "string",
        ) &&
        typeof item.startLine === "number" &&
        typeof item.endLine === "number" &&
        Array.isArray(item.applicability) &&
        item.applicability.every((scope) => typeof scope === "string"),
    )
  ) {
    return {
      status: "ready",

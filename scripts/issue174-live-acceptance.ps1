...
$expectedPendingCount = if ($IntakeDiagnostic) { 5 } else { 25 }
...
if (-not $IntakeDiagnostic) {
    ...
    if ($RunId -ne 'issue174-live-07' -or $pending.Count -ne 25) { throw '须使用冻结运行标识并保留二十五笔 PENDING。' }
...
$reportPath = Join-Path $repoRoot $(if ($InvestigationDiagnostic) { 'docs/delivery/issue-174-investigation-diagnostic-01-report.json' } else { 'docs/delivery/issue-174-live-report-07.json' })

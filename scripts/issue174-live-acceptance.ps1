...
$expectedPendingCount = if ($IntakeDiagnostic) { 5 } else { 28 }
...
if (-not $IntakeDiagnostic) {
    ...
    if ($RunId -ne 'issue174-live-10' -or $pending.Count -ne 28) { throw '须使用冻结运行标识并保留二十八笔 PENDING。' }
...
$reportPath = Join-Path $repoRoot $(if ($InvestigationDiagnostic) { 'docs/delivery/issue-174-investigation-diagnostic-01-report.json' } else { 'docs/delivery/issue-174-live-report-10.json' })
...
function Assert-FrozenCatalog {
    ...
    if (
        ...
        $catalog.revision -ne 10 -or
        $catalog.runId -ne $RunId -or
        $catalog.liveFreeze.promptVersionsBySeam.intake -ne 'intake-v3' -or
        $catalog.liveFreeze.promptVersionsBySeam.action -ne 'investigation-action-v5' -or
...
function Assert-LiveUsage($Usage, [switch]$RequireAllSeams) {
    ...
    if ($RequireAllSeams) {
        if (
            ...
            $Usage.Formal.action.promptVersion -ne 'investigation-action-v5' -or
...
"@ | Set-Content -LiteralPath $overridePath -Encoding utf8
    if (-not $IntakeDiagnostic -and -not $InvestigationDiagnostic) {
        # 只追加审计元数据输出，保留原方法、响应解析与异常传播；不保存原始正文。
        $auditSourcePath = Join-Path $evidenceDir 'deepseek_investigation_model.py'
        $auditSource = Get-Content (Join-Path $repoRoot 'agent/src/baseline_agent/deepseek_investigation_model.py') -Raw
        $auditSource += @'
...
async def _issue174_record(self, record):
    await _original_issue174_record(self, record)
    item = {
        "callId": record.internal_call_id,
        "attempt": record.attempt_number,
        "promptVersion": record.prompt_version,
        "httpStatus": record.provider_http_status,
        "responseStatus": record.response_status,
        "failure": record.failure_classification.value if record.failure_classification else None,
    ...
    if (-not $IntakeDiagnostic -and $reservationWritten -and $providerMayHaveRun -and $artifactsAvailable) {
        try {
            ...
            $failureReport = [ordered]@{
                ...
                springRejections = $usage.Database.springRejections
                cost = [ordered]@{

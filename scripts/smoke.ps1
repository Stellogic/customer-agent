...
docker build --build-arg "GRADLE_OPTS=$($env:CUSTOMER_AGENT_GRADLE_OPTS)" --target test --tag customer-agent/backend-test:issue15 backend
docker build --target test --tag customer-agent/agent-test:issue15 agent
docker build --target test --tag customer-agent/frontend-test:issue15 frontend
...
if (($migrationHistory -join ',') -ne '1:true,2:true,3:true,4:true') {
...
docker run --rm --entrypoint sh customer-agent/frontend:issue15 -c "if grep -R -E 'agent-server|agent:2024|local-(spring|agent|executor|postgres)|postgresql://' /usr/share/nginx/html; then exit 1; fi"

$versions = [ordered]@{
    node = (docker run --rm node:24.19.0-bookworm-slim node --version 2>$null)
    java = (docker run --rm --entrypoint sh customer-agent/backend:issue15 -c 'java -version 2>&1 | head -n 1')
    python = (docker run --rm --entrypoint python customer-agent/agent:issue15 --version 2>&1)

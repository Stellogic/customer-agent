...
本仓库正在实现“客服工单调查与补偿审批 Agent”。当前代码已贯通客户创建工单、Agent 调查、无需补偿时自主解决、按物流延迟政策生成等待人工审批的不可变补偿提案版本、审批人最小队列与限时排他领取/审批视图，以及订单歧义时在同一处理代次和 thread 内暂停、澄清与恢复。Spring 还会按可控业务时钟形成首次响应和解决 SLA 的不可变预警/违约事实：预警仅通知当前负责客服，违约只把最小摘要投影到客服共享升级队列。审批决定与补偿执行仍由后续切片实现。
...
- 审批人共享队列只暴露提案编号、方式、金额和有效期；领取使用稳定请求身份与限时排他租约。只有当前 `leaseToken/leaseVersion` 持有人能读取按提案版本裁剪的 `APPROVAL_VIEW`，释放、过期或提案替换会立即撤销读写资格；本切片不包含审批决定或补偿执行入口。

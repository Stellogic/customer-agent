...
2026-08-31，与 #169 负责人直接确认。状态为**限定静态实现，NOT_RUN**；不是现行 HTTP API，也不是后端授权已完成。基线 `origin/main@c19a7ebe8ec31f7ed21048ea75fbfcfd61df1472`，保留既有 Draft PR #209。
...
新增纯状态测试源码覆盖稳定请求重放、异类型拒绝、旧请求迟到、撤权停止、重新领取及旧 assignment 拒绝隔离；更新组件测试覆盖实际 state 到草稿清屏、引用元数据展示、普通失败保留编辑及两类模型失败提示。全部 **NOT_RUN**。

未运行测试、格式/类型/lint、构建、Docker、浏览器、模型、评测或完整门禁；未申请/查询/持有测试锁。真实消费仍需 #190 冻结质量 PASS、完整门禁、合入关票及协调明确放行，再同步 main、补齐后端链路与增量双 CR。CI 关闭，外部审查不阻塞。任何已收到且有证据的问题仍正常修复。

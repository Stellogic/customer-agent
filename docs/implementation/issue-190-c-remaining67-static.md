...
固定差异 `669f640...1252495`，源码提交 `1252495a19058115bcce28a1c16471abbbf96f6d`。任何静态PASS都不代替本轮尚未获准的运行验证。

### Standards

PASS（静态），0项缺陷。新增入口仅选择原序第6–72项，以固定清单、独立phase及既有账本快照约束范围；保留原两阶段状态、历史费用及累计6元上限。发送前逐项核对ID与请求hash，首次错误停止，新RunId不能重开；成功也不计算质量指标。复用原构造、解析、取证和结算路径，无通用续跑框架。文档明确逐请求hash待获准离线物化、新增验证未运行。审阅未执行检查、查询锁或读取留出/#189。

### Spec

PASS（静态），0项缺陷。实现仅取原序第6–72项，并核对固定清单顺序；新阶段绑定完整历史账本，保留原STOPPED和单题诊断记录。逐请求核对ID/hash、沿用累计预算及异常停止逻辑；新RunId不可重开。成功仅标记诊断完成，metrics=null，不评分或解阻。文档明确67项请求字节hash尚待获准离线物化，新增测试未运行，未虚称验证通过。审阅未运行检查、查询锁或读取留出/#189。

两轴分别0项未决。本阶段结束状态仍为CODE_READY_NO_TESTS，无新增RunId、质量结果或LOCK_RELEASED事件，因为本轮没有占锁运行。

...
客户最终发布新增 `revalidateCustomerForPublication(AgentKnowledgeResult receipt)`：MANDATORY，要求调用方已有工单写事务与授权锁；不编码，先取得与目录/向量索引更新相同键16620260829的共享事务 advisory lock，再复核回执，保证复核至公开消息写入之间不会穿插重建。原四方法和 #170 消费契约不变。此新增方法不用于长时模型调用；只包住最终短事务。
...
#169 修改 knowledge 适配/投影、客户 investigation/Agent graph/communication、公开消息和 App.tsx；#170 修改自有 queue/HUMAN/SupportWorkbench，拥有 langgraph.json 的 support_assistance 图注册与 V44 迁移。按协调顺序 #169 先交付，使用 V43__customer_knowledge_reply.sql；#168 独占 encoder 公开加载、ONNX、消融和相应依赖增量，本票不改 encoder/pyproject/uv.lock。

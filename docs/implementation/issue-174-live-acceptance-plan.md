...
原五场景、action v6/schema v3、预算/次数上限和首次失败即停规则不变。Backend规范化check通过（日志.local/issue174-confirm-button-focused2.log），双轴静态审查通过，本轮已执行，整轮预留保留；最终完整门禁NOT_RUN。以下为历史快照。
...
Agent 聚焦检查470 passed/3 skipped，格式、lint、类型检查及双轴静态审查通过。沿用仅受控字段的拒绝观察器，不保存原始载荷。本轮已结束并保留整轮预留，禁止重跑；最终完整门禁 NOT_RUN。以下为历史快照。

状态：**第9版 INCOMPLETE：重复扣款结论的证据适用性被 Spring 拒绝**。见[第9轮结果](../delivery/issue-174-live-09-result.md)。

## 第9版复验冻结

用户明确允许公开第8轮技术验收记录，并追加1元进行一次带拒绝码证据的复验。总授权提高至12元，27笔旧PENDING和旧失败保留；历史核对上界2元加后续预留8.9元为10.9元，本轮另预留最多1元后11.9元。费用预留和程序估算均不代表平台实扣。

入口`pwsh ./scripts/issue174-live-acceptance.ps1 -ConfirmProviderSpend -RunId issue174-live-09`，见[调用前冻结](../delivery/issue-174-live-09-freeze.json)。原五场景、模型、prompt/schema、费用和次数上限、首次失败即停规则均不变。本轮不新增产品修复；#226已有修复保留。

runner在清理前保存Spring拒绝事件码计数，运行时观察器仅记录结论类型、回复schema/意图、字段一致性、证据数量、正文长度和数值声明等受控信息到本地忽略目录。不保存完整结论请求、响应正文、订单标识或认证头；不放宽原始载荷禁止持久化边界。模型调用前验证观察器导入及目录写入；原请求、返回值和异常传播不变。第8轮仍为INCOMPLETE，不回填缺失证据。

本轮已结束，整轮预留保留，禁止重跑；#174 最终完整门禁 NOT_RUN。以下为调用前及历史快照。

状态：**第8版 INCOMPLETE：第一场景浏览器通过，Spring结论拒绝后停止**。见[第8轮结果](../delivery/issue-174-live-08-result.md)。
...
本轮继续保存本地逐次审计元数据；不保存原始供应商正文和密钥。本轮已结束并保留预留，禁止重跑；#174最终完整门禁NOT_RUN，以下均为历史记录。

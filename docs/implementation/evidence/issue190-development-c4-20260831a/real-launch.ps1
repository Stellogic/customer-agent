...
$runId = 'issue190-development-c4-20260831a'
$ledgerPath = 'D:/customer-agent/.local/issue190-sufficiency/cost-ledger.json'
if ((Get-FileHash -LiteralPath $ledgerPath -Algorithm SHA256).Hash.ToLowerInvariant() -ne '6aaf63e0b38110df34a98138c18ae2c86156a39f2d5310791fbe15a4a9a12693') { throw 'SHARED_LEDGER_CHANGED; no API call' }
...
$metadata = [ordered]@{
    ...
    budget_micro_cny=6000000; maximum_attempts_per_query=1; maximum_api_requests=72; seen_development_only=$true; prior_settled_upper_micro_cny=202485;
    input_cny_per_million_upper=3; output_cny_per_million_upper=9;
    fixed_context_upper=1048576; max_output_tokens=256
}
$watch = [Diagnostics.Stopwatch]::StartNew()
try {
    & "$root/scripts/knowledge-sufficiency.ps1" -DevelopmentVersion c4 -RunId $runId -PricingAndContextVerifiedDate $observedDate -Uv "$root/.local/tools/uv/uv.exe" *> "$evidence/launch.log"

...
本票已拆为 #189、#190，只保留范围与验收汇总职责，不再作为独立实施票领取或重复创建实现 PR。原始验收清单完整保留。

| 原验收条目（按下文顺序） | 承接子票与证据 |
| --- | --- |
| 1–2：预冻结评测清单、覆盖和内容哈希 | #189 |
| 3：固定阈值及实际通过 | #189 冻结阈值；#190 提供不可改题的质量实测通过证据 |
| 4：模型与 Embedding 协议、文件清单 | #189 冻结协议；#190 准备并校验模型文件、实现协议 |
| 5–10：离线模型、PyTorch、硬过滤/RRF、精确检索、真实页面及失败行为 | #190 |

关闭前必须确认 #189、#190 均正式交付，并逐项核对上述证据。#190 必须通过冻结质量实测及最终本地完整门禁、PR 合入 main 并关闭 Issue；静态双审 PASS、CODE_READY_NO_TESTS 或 GATE_READY 均不能替代。

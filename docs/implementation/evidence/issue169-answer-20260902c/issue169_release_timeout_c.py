...
expected_before = "47a691aa1d262443c6a2a77a5efd8aba9bd79af9e796f251a5dda879c31e0bd5"
...
targets = [
    ...
    if item.get("phase") == "issue169-answer-20260902c"
...
if len(targets) != 1 or targets[0] is not state["attempts"][-1]:
    raise SystemExit(f"expected latest timeout reservation, found {len(targets)}")
...
target["release"] = {
    ...
    "authorized_retry_run": "issue169-answer-20260902d",
}
...
evidence = {
    ...
    "authorized_retry_run": "issue169-answer-20260902d",
    "note": "用户在连接根因修复并完成离线双CR后，授权释放连接超时的未知usage预留；供应商未确认不计费。",
}

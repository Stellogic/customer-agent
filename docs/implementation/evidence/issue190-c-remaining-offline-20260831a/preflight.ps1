...
try {
    ...
    try {
        ...
        $env:NO_PROXY = '*'
        $parseCommand = '$tokens=$null; $errors=$null; [System.Management.Automation.Language.Parser]::ParseFile("../scripts/knowledge-sufficiency.ps1",[ref]$tokens,[ref]$errors) | Out-Null; if($errors.Count){ $errors; exit 1 }; "PowerShell entry syntax PASS"'
        Invoke-Phase 'entry-syntax' 'pwsh' @('-NoProfile','-Command',$parseCommand)
        ...
        Invoke-Phase 'materialize-requests' $python @("$root/.local/issue190-materialize-remaining.py",'--output',"$out/materialized",'--run-id',$RunId)
        $record.status = 'PASS'

...
$runId = 'issue190-development-c5-20260831a'
$ledgerPath = 'D:/customer-agent/.local/issue190-sufficiency/cost-ledger.json'
if ((Get-FileHash -LiteralPath $ledgerPath -Algorithm SHA256).Hash.ToLowerInvariant() -ne 'ed0ec71c983c076974fa0254e9222fe99f5efa1ec6b564f7cc0b74a904928933') { throw 'SHARED_LEDGER_CHANGED; no API call' }
...
$metadata = [ordered]@{
    ...
    budget_micro_cny=6000000; maximum_attempts_per_query=1; maximum_api_requests=72; seen_development_only=$true; prior_settled_upper_micro_cny=287133;
    input_cny_per_million_upper=3; output_cny_per_million_upper=9;
    fixed_context_upper=1048576; max_output_tokens=256
}
$watch = [Diagnostics.Stopwatch]::StartNew()
try {
    & "$root/scripts/knowledge-sufficiency.ps1" -DevelopmentVersion c5 -RunId $runId -PricingAndContextVerifiedDate $observedDate -Uv "$root/.local/tools/uv/uv.exe" *> "$evidence/launch.log"

...
## 2026-09-01 连续验证窗口恢复记录

协调任务 `01a043aa-d724-7353-b6c5-9266277846d6` 明确重申：#169 唯一连续运行/正式交付授权有效，覆盖最初静态禁测限制。继续聚焦验证、真实 PostgreSQL/浏览器、客户回答质量、双轴审查、最终完整门禁与合入关票；无须逐阶段申请。36+12 客户集及8项边界必须调用前冻结审查，沿原累计账本不超过6元。锁占用/恢复规则不变；CI关闭。

恢复点为 HEAD `915fb683ae7abc0ddabdd40f7297fc35d1f4afb7`，已整合 main `e34b60113a3bbcfe28a4fcd247900127ffbd234a`。focus5–13 和 runtime-a–d 在该提交加未提交改动上执行，**不是该提交原样验证**。保留所有失败日志：Python修复后的132项、Java139项、前端63项聚焦通过；runtime-d真实HTTP/PG18项和V43事务SQL通过，浏览器因本机缺指定Chromium未启动。模型调用仍为0，后续运行单独记证据。

真实服务揭示并修复两个问题：Markdown允许的适用范围漏掉CUSTOMER_PUBLIC，现只补该已有协议枚举；常规客户知识追问被旧业务自动处理白名单转人工，现保留硬性人工条件、允许知识会话继续，但不赋予自动关闭资格。均保留原失败再验证，不以fixture结果充当模型质量。

## 调用前复核与新增运行记录

独立 Spec / Standards 对客户集及执行器 PRE_CALL PASS：36条有答案对应18个明确公开命题（每命题两种问法，不宣称36种独立规则），12条无答案未被公开语料覆盖。修复并复核了数据路径未绑定冻结清单、修正前回答/拒绝结果留存，以及有效回答但未知usage时立即停止三项执行器问题。所有48行在运行前预登记；保留每次compose、实际供应商流、费用预留和故障阶段。

全产品双轴审查发现相同P2：无引用状态跳过内部ID阻断。现统一检查完整回执的articleId/chunkId，最终Spring接受前检查整个公开正文；补负例。复核 Standards PASS / Spec PASS，无已知未解决静态发现。focus15 Java140项及Spotless/Checkstyle通过；focus14 Python132项及格式/lint/类型通过。上述仍为915fb68加工作树改动，不是最终门禁。

runtime-e HTTP/PG18项、V43 SQL事务校验及Edge1440/390页面来源/断线恢复2项通过，资源清理PASS且LOCK_RELEASED。浏览器使用主机已安装Edge，不能替代最终门禁的项目固定浏览器版本。早期全页截图未把视口外来源完整捕获，后续改为滚动来源组件并截图，不以空白截图声称视觉通过。

截至本冻结候选，原账本再次读回289 SETTLED、0 PENDING、累计620805微元，真实模型0次。整个客户回答质量、完整Agent自主检索个案及最终完整门禁仍未通过；PRE_CALL PASS不等于回答质量PASS。

## 连续窗口暂停：runtime-f 隔离事故

2026-09-01 runtime-f的执行器预检顺序错误触发默认baseline清理，原日志确认baseline容器/网络/数据卷被删除。事故报告及原始记录见 `evidence/issue169-runtime-20260901f/incident.md`；原错误PASS完整保留，并另追加更正。协调已暂停所有运行，由协调向用户说明数据损失和恢复选择。本任务没有擅自重建、清理其他资源或调用模型。当前只作静态执行器修复与独立复核，真实质量、新增边界、最终门禁与合入关票全部暂停。不能报告整票CODE_READY或验证完成。

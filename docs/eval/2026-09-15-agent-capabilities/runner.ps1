param(
    ...
    [Parameter(Mandatory)][ValidateRange(1, 10000000)][long]$LimitMicros,
    [Parameter(Mandatory)][ValidateRange(1, 80)][int]$MaxAttempts,
    [Parameter(Mandatory)][ValidateRange(1, 160000)][int]$MaxTokens,
    [Parameter(Mandatory)][datetimeoffset]$Deadline,
    [Parameter(Mandatory)][ValidateRange(1, 600000)][int]$InvestigationWallClockMs,
    [string]$RunId = ('issue242-capabilities-' + [guid]::NewGuid().ToString('N').Substring(0, 8))
)
...
$repo = 'D:/customer-agent'
$evaluationDirectory = $PSScriptRoot.Replace('\', '/')
...
if (-not $ContinueOnCaseFailure) { throw '本轮冻结为普通案例失败继续，必须传入 ContinueOnCaseFailure。' }
if ($LimitMicros -ne 10000000 -or $MaxAttempts -ne 80 -or $MaxTokens -ne 160000 -or $InvestigationWallClockMs -ne 60000) { throw '本轮限额冻结为10元、80次、160000 token、调查动作循环60000ms。' }
if ([string]::IsNullOrWhiteSpace($env:DEEPSEEK_API_KEY)) { throw '当前进程缺少 DEEPSEEK_API_KEY；不会读取 .env。' }
if ($RunId -notmatch '^issue242-capabilities-[a-z0-9-]+$') { throw 'RunId 必须使用 issue242-capabilities- 前缀。' }
if ($TestedHead -notmatch '^[0-9a-f]{40}$' -or (git rev-parse HEAD).Trim() -ne $TestedHead -or @(git status --porcelain --untracked-files=no).Count -ne 0) {
...
if ($Deadline -gt [datetimeoffset]::UtcNow.AddMinutes(45)) { throw '本轮截止时间不得晚于当前时间45分钟后。' }
...
$gate = Enter-TestGateLock -Issue 242 -RunId $RunId -CommandType 'real-core-browser' `
...
function Read-MatrixResults($Suite) {
    foreach ($spec in $Suite.specs) {
        if ($spec.title -match '^Agent capabilities 20260915 ([a-z0-9_]+)$') {
            $name = $Matches[1]; $number = 1
...
try {
    ...
    [ordered]@{
        ...
        denominator = $matrix.Count; matrix = $matrix; retries = 0; maxFailures = $maximumCaseFailures; continueOnCaseFailure = $ContinueOnCaseFailure.IsPresent; workers = 1
        sideEffects = @('创建本轮合成订单与工单', '发布客户公开回复及后续补充', '创建待审批提案；不批准或执行补偿')
...
services:
  ...
  agent-server:
    ...
    environment:
      ...
      AGENT_INVESTIGATION_MAX_ACTIONS: 12
      AGENT_INVESTIGATION_MAX_PROVIDER_ATTEMPTS: 12
      AGENT_INVESTIGATION_MAX_TOKENS: 16000
      AGENT_INVESTIGATION_MAX_COST_MICROS: 500000
      AGENT_INVESTIGATION_MAX_REPEATED_ACTIONS: 0
  ...
  core-budget-stop:
    image: customer-agent/agent:$tag
    entrypoint: [python, -c]
    command: ['import sys; from pathlib import Path; from baseline_agent.core_validation_budget import CoreValidationBudget; CoreValidationBudget.open(Path(sys.argv[1]), authorization_id=sys.argv[2]).stop(sys.argv[3])', '/core-budget/$ledgerName', '$AuthorizationId', AGENT_CAPABILITY_MATRIX_FINISHED]
    volumes:
      - '${ledgerDirectory}:/core-budget'
    networks: [data]
  core-metrics:
    image: customer-agent/agent:$tag
    entrypoint: [python, /evaluation/collect.py]
    ...
    volumes:
      ...
      - '${mount}:/core-evidence'
      - '${evaluationDirectory}:/evaluation:ro'
  ...
  browser-acceptance:
    ...
    volumes:
      ...
      - '${ledgerDirectory}:/core-budget:ro'
      - '${evaluationDirectory}:/evaluation:ro'
      - '${evaluationDirectory}/evaluation.spec.ts:/app/e2e/evaluation.spec.ts:ro'
      - '${repo}/frontend/src:/app/src:ro'
      - '${repo}/frontend/tsconfig.json:/app/tsconfig.json:ro'
      - '${repo}/frontend/vite.config.ts:/app/vite.config.ts:ro'
      - '${repo}/frontend/.prettierrc.json:/app/.prettierrc.json:ro'
      - '${repo}/frontend/eslint.config.js:/app/eslint.config.js:ro'
"@ | Set-Content -LiteralPath $override
    ...
    Invoke-RealCompose @('run', '--rm', '--no-deps', '--entrypoint', 'npx', 'browser-acceptance', '--no-install', 'prettier', '--check', 'e2e/evaluation.spec.ts')
    Invoke-RealCompose @('run', '--rm', '--no-deps', '--entrypoint', 'npx', 'browser-acceptance', '--no-install', 'eslint', '--max-warnings', '0', 'e2e/evaluation.spec.ts')
    Invoke-RealCompose @('run', '--rm', '--no-deps', '--entrypoint', 'npx', 'browser-acceptance', '--no-install', 'tsc', '--noEmit')
    if ($Deadline -le [datetimeoffset]::UtcNow) { throw '构建后已超过冻结截止时间，禁止开始真实调用。' }
    $started = $true
    Invoke-RealCompose @('run', '--rm', '--no-deps', '--entrypoint', 'python', 'core-metrics', '-c', 'import runpy; runpy.run_path("/evaluation/collect.py", run_name="preflight"); print("METRICS_MODULE_IMPORT_OK")')
    Invoke-RealCompose @('up', '-d', '--no-build', '--wait', 'backend', 'agent-server', 'browser-frontend')
    $servicesReady = $true
    # 不启动 compensation-executor，场景也不执行审批或赔付操作。
    Invoke-RealCompose @('run', '--rm', '--no-deps', 'browser-acceptance', '--workers=1', '--retries=0', "--max-failures=$maximumCaseFailures", '--reporter=json', '--output=/artifacts', 'e2e/evaluation.spec.ts')
    ...
    try {
        ...
        try {
            ...
            if (Test-Path -LiteralPath $evidence) {
                ...
                $allPassed = @($matrix | Where-Object status -eq 'PASS').Count -eq $matrix.Count
                ...
                $automaticChecksPassed = ($allPassed -and $metricsComplete -and $cleanupPassed -and $unsettled -eq 0 -and $outcome -eq 'BROWSER_PASS')
                [ordered]@{
                    runId = $RunId; head = $TestedHead; authorizationId = $AuthorizationId
                    outcome = $outcome; denominator = $matrix.Count; matrix = $matrix
                    metricsCollected = $metricsCollected; cleanupPassed = $cleanupPassed
                    automaticChecksPassed = $automaticChecksPassed; semanticReviewStatus = 'PENDING'; capabilityAcceptance = $null
...
if (-not $automaticChecksPassed) { throw '核心矩阵或计量未完整通过；保留失败和未运行分母，不追加真实运行。' }

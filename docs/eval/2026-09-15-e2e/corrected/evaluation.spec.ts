...
type BusinessSnapshot = {
  tickets: Ticket[];
  commands: { generationId: string; operation: string; response: { accepted?: boolean } }[];
  executions: number;
    ...
    try {
      ...
      for (const ticket of tickets) {
        ...
        if (scenario.expected === "human_no_proposal") {
          ...
          const completedGeneration = state.generations.find((g) => g.ticket_id === ticket.id)!;
          expect(
            state.commands.filter(
              (c) =>
                c.generationId === completedGeneration.id &&

...
$evaluationDirectory = 'D:/customer-agent/.local/eval-20260915-corrected'
...
function Read-MatrixResults($Suite) {
    foreach ($spec in $Suite.specs) {
        if ($spec.title -match '^E2E 20260915 ([a-z0-9_]+)$') {
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $evaluationDirectory 'scenarios.json'), (Join-Path $evaluationDirectory 'new-e2e.spec.ts'), $PSCommandPath -Destination $evidence
    [ordered]@{
        schemaVersion = 'customer-agent-e2e-20260915-v1'; scenarios = (Get-Content -Raw (Join-Path $evaluationDirectory 'scenarios.json') | ConvertFrom-Json); authorizationId = $AuthorizationId
        ...
        denominator = 8; matrix = $matrix; selectedCase = $Case; selectedSample = $Sample; retries = 0; maxFailures = $maximumCaseFailures; continueOnCaseFailure = $ContinueOnCaseFailure.IsPresent; workers = 1
...
services:
  ...
  browser-acceptance:
    ...
    volumes:
      ...
      - '${evaluationDirectory}/new-e2e.spec.ts:/app/e2e/new-e2e.spec.ts:ro'
...
"@ | Set-Content -LiteralPath $override
    ...
    Invoke-RealCompose @('run', '--rm', '--no-deps', '--entrypoint', 'npx', 'browser-acceptance', '--no-install', 'prettier', '--check', 'e2e/new-e2e.spec.ts')
    Invoke-RealCompose @('run', '--rm', '--no-deps', '--entrypoint', 'npx', 'browser-acceptance', '--no-install', 'eslint', '--max-warnings', '0', 'e2e/new-e2e.spec.ts')
    ...
    $started = $true
    Invoke-RealCompose @('run', '--rm', '--no-deps', '--entrypoint', 'python', 'core-metrics', '-c', 'import baseline_agent.core_validation_metrics; print("METRICS_MODULE_IMPORT_OK")')
    Invoke-RealCompose @('up', '-d', '--no-build', '--wait', 'backend', 'agent-server', 'browser-frontend')
    $servicesReady = $true
    # 不启动 compensation-executor，场景也不执行审批或赔付操作。
    Invoke-RealCompose @('run', '--rm', '--no-deps', 'browser-acceptance', '--workers=1', '--retries=0', "--max-failures=$maximumCaseFailures", '--reporter=json', '--output=/artifacts', 'e2e/new-e2e.spec.ts')
    ...
    try {
        ...
        try {
            ...
            if (Test-Path -LiteralPath $evidence) {
                ...
                $allPassed = @($matrix | Where-Object status -eq 'PASS').Count -eq 8
                ...
                [ordered]@{
                    ...
                    outcome = $outcome; denominator = 8; matrix = $matrix

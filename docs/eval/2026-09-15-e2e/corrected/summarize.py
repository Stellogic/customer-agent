...
OUT = ROOT / 'docs/eval/2026-09-15-e2e/corrected'
...
summary = {'runId':'issue230-real-eval-20260915-corrected','result':read(RAW/'result.json'),
...
summary['ledgerTotals'] = {
    'source':'persistent-budget-ledger-and-independent-metrics',
    ...
    'independentMetricsReconciliation':metrics.get('knownProviderAttempts')==len(attempts) and metrics.get('unknownUsageAttempts')==0 and metrics.get('estimatedCostMicros')==sum(a.get('estimatedCostMicros') or 0 for a in attempts),
}

...
本阶段仅编写消融、ONNX 导出/张量执行、一致性比较、资源基准与独立测试源码。没有运行测试、格式/类型命令、构建、下载、导出、推理或评测。没有 PyTorch 质量结论、ONNX 指标或参数冻结结论，也没有默认切换。报告模板中的 `null` 表示未运行，不是零。
...
测试源码覆盖三路/冻结题调度、部分失败证据、排序逆序/版本差异/单边拒答、非有限输出、真实子进程计时与 RSS 边界，以及小型合成 feature model 的真实 ONNX 动态维度导出。合成图测试缺可选依赖会 skip；这不是 BGE 质量/性能证据，正式 ONNX 验收不允许用 skip 代替通过。所有这些测试在本阶段均未运行。

...
issue217-live-20260905c 仍测试 cad8d6d，仅增加清理前读取现有Spring拒绝事件计数。确认建单断言再次通过，但第7次行动失败为INVALID_JSON，未调用业务判断或客户沟通，未到达Spring结论接收；拒绝事件计数为空。此结果不能解释b轮422，也不能证明1024额度已解决全部输出问题。保留本轮失败，不循环重跑浏览器。

已知4988token，调查usageTrusted=true；0.057941元为已知用量与受理保守估算，非实扣，整轮0.2元保留PENDING。现16笔PENDING，核对点后预留2.5元，保守剩余1.3元。本轮精确资源清理回读为空，TEST_GATE_FREE，基线执行器仍运行。下一步缩小为单次合成结论选择的结构诊断，避免全链路重复调用。PR已首次转Ready开展集中审查，最终完整门禁仍NOT_RUN。

...
先由主方案确定 C 的网络/版本/数据边界是否可接受；若接受，以**一个固定模型、一个固定判定合同、在任何调用前锁定的提示词**做小规模可行性验证，不从 A 已见结果增加领域规则，不把冻结题作提示示例。开发反馈后不修改提示重试；开发通过后仅冻结受测源码、提示和报告SHA交给独立验证。新的路线仍需独立保留数据验证；已经打开的材料不能再次冒称未见，冻结原门槛完全保留。

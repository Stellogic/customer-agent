...
4. 客户 SSE 断线后页面清除旧内容，连接恢复后只从 Spring 权威快照重新呈现公开沟通；既有跨角色矩阵继续验证分配撤销和审批租约失效后的即时清屏。
5. 客户与客服浏览器请求路径、请求体、API 响应、原始 SSE data 和生产全部 JavaScript/CSS bundle 不包含模型密钥标识、Agent 私有地址、模型输入分区、原始响应、工具 payload 或 checkpoint。

CREATE TABLE agent_public_reply_stream (
    generation_id uuid PRIMARY KEY REFERENCES agent_processing_generation(id),
    ticket_id uuid NOT NULL REFERENCES support_ticket(id),
    status text NOT NULL CHECK (status IN ('LOADING', 'STREAMING', 'COMPLETED', 'ABORTED', 'FAILED')),
    ...
    progress_stage text CHECK (
        progress_stage IS NULL OR progress_stage IN (
            'UNDERSTANDING', 'VERIFYING_FACTS', 'QUERYING_RULES', 'COMPOSING_REPLY'
        )
    ),
    updated_at timestamptz NOT NULL,
    UNIQUE (ticket_id, generation_id)
);

CREATE TABLE agent_public_reply_event_request (
...
GRANT SELECT, INSERT, UPDATE ON agent_public_reply_stream TO spring_app;
GRANT SELECT, INSERT ON agent_public_reply_event_request TO spring_app;

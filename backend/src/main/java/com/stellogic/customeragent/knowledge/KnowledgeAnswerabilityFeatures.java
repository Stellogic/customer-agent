...
final class KnowledgeAnswerabilityFeatures {
    static final List<String> NAMES =
            List.of("dense_max", "dense_margin", "query_term_coverage", "retriever_agreement");
    ...
    static List<Double> extract(
            ...
            List<KnowledgeRetrievalHit> fused) {
        ...
        Set<String> left =
                new HashSet<>(
                        lexical.stream().limit(5).map(KnowledgeRetrievalHit::chunkId).toList());
        Set<String> right =
                new HashSet<>(dense.stream().limit(5).map(KnowledgeRetrievalHit::chunkId).toList());

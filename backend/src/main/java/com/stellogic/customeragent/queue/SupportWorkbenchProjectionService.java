...
class SupportWorkbenchProjectionService {
    ...
    SupportWorkbenchSnapshot snapshot(String supportId) {
        ...
        Long sequence = jdbc.queryForObject(
                ...
                Long.class,
                EPOCH);
        return new SupportWorkbenchSnapshot(EPOCH, sequence == null ? 0 : sequence, shared, escalations);
    }
    ...
    List<SupportWorkbenchEvent> events(String supportId, String afterCursor) {
        ...
        return jdbc.query(
                ...
                (rs, row) -> new SupportWorkbenchEvent(
    ...
    SupportTicketDetails details(String supportId, UUID ticketId) {
        requireSupport(supportId);
        List<SupportTicketDetails> tickets = jdbc.query(
        ...
        List<SupportInvestigationFact> facts = jdbc.query(
                ...
                (rs, row) -> new SupportInvestigationFact(
                        rs.getString(1), rs.getString(2), rs.getString(3), rs.getTimestamp(4).toInstant()),
                ticketId);
        List<SupportTimelineEvent> timeline = jdbc.query(
                "select event_type, actor_id, occurred_at from audit_event where ticket_id = ? order by occurred_at, id",
                (rs, row) -> new SupportTimelineEvent(
                        rs.getString(1), rs.getString(2), rs.getTimestamp(3).toInstant()),
                ticketId);
        return new SupportTicketDetails(
                ticket.ticketId(), ticket.customerId(), ticket.orderReference(), ticket.description(),
                ticket.lifecycleState(), ticket.handlingMode(), conversation, facts, timeline);
    }

...
class SupportAssistanceService {
    ...
    SupportAssistanceService(
            SupportAssistanceRequests requests,
            SupportAssistanceContext context,
            AgentKnowledgeRetrievalAdapter knowledge,
            SupportAssistanceGateway gateway,
            ObjectMapper json) {
    ...
    JsonNode request(String supportId, UUID ticketId, SupportAssistanceRequest request) {
        ...
        try {
            generated =
                    gateway.generate(request.kind(), request.query(), receipt.input(), retrieved);
        } catch (RuntimeException failure) {
            requests.recordModelAudit(
                    supportId,
                    request.requestId(),
        ...
        requests.recordModelAudit(
                supportId, request.requestId(), json.writeValueAsString(generated.path("audit")));
        if (!"completed".equals(generated.path("status").asString())) {
            String code =
                    "INVALID_ANSWER_FORMAT".equals(generated.path("code").asString())
        ...
        try {
            knowledge.revalidateSupport(
                    supportId,
                    json.treeToValue(stored.path("knowledge"), AgentKnowledgeResult.class));
        } catch (RuntimeException failure) {
            saveFailure(supportId, receipt, "RETRIEVAL_UNAVAILABLE", generated.path("audit"));
            throw failure;
        }
        requests.finish(
                supportId, ticketId, request.requestId(), json.writeValueAsString(stored), false);
        return result(supportId, ticketId, request.requestId());
    }
    ...
    JsonNode result(String supportId, UUID ticketId, UUID requestId) {
        ...
        if (receipt.resultJson() == null)
            return envelope(receipt, Map.of("status", "loading", "kind", receipt.kind().name()));
    ...
    private JsonNode project(
            SupportAssistanceReceipt receipt,
            JsonNode raw,
            AgentKnowledgeResult retrieved,
            JsonNode audit) {
        if (!raw.isObject()
        ...
        if (answer == null
                || !List.of("SUPPORTED", "INSUFFICIENT_INFORMATION").contains(answer.decision())
                || answer.text() == null
                || answer.text().isBlank()
        ...
        for (var quote : answer.citations()) {
            var source =
                    retrieved.results().stream()
                            .filter(item -> item.chunkId().equals(quote.chunkId()))
                            .findFirst()
                            .orElseThrow(
                                    () ->
                                            new IllegalArgumentException(
                                                    "citation is outside this request"));
            if (quote.quote() == null
                    || quote.quote().isBlank()
                    || !source.snippet().contains(quote.quote())) {
        ...
        return json.valueToTree(
                Map.of(
                        ...
                        view,
                        "knowledge",
                        new AgentKnowledgeResult(
                                retrieved.schema(), retrieved.indexGeneration(), selected),
                        "audit",
                        audit));
    }

    private void saveFailure(
            String supportId, SupportAssistanceReceipt receipt, String code, JsonNode audit) {
        String reason =
                switch (code) {
        ...
        requests.finish(
                ...
                json.writeValueAsString(
                        Map.of(
                                "view",
                                Map.of("status", "error", "reason", reason),
                                "audit",
                                audit)),
                true);
    }

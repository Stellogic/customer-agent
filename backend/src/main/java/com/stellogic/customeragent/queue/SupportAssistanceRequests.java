...
class SupportAssistanceRequests {
    ...
    SupportAssistanceRequests(
            JdbcTemplate jdbc, SupportAssistanceContext context, ObjectMapper json) {
        this.jdbc = jdbc;
    ...
    @Transactional
    SupportAssistanceReceipt begin(
            String supportId, UUID ticketId, SupportAssistanceRequest request) {
        lockTicket(ticketId);
        var input = context.load(supportId, ticketId);
        if (!input.assignmentId().equals(request.assignmentId()))
        ...
        if (!stored.ticketId().equals(ticketId)
                || !stored.assignmentId().equals(request.assignmentId())
                || stored.kind() != request.kind()
                || !stored.query().equals(request.query())) {
    ...
    void recordModelAudit(String supportId, UUID requestId, String auditJson) {
        ...
        jdbc.update(
                "update support_assistance_request set model_audit = ?::jsonb where support_id = ? and request_id = ?",
                auditJson,
                supportId,
                requestId);
    }
    ...
    @Transactional
    void finish(
            String supportId, UUID ticketId, UUID requestId, String resultJson, boolean failed) {
        lockTicket(ticketId);
        var stored = find(supportId, requestId);
        if (!stored.ticketId().equals(ticketId)) throw new SupportTicketNotFoundException();
        context.requireAssignment(supportId, ticketId, stored.assignmentId());
                ...
                """,
                        (rs, row) ->
                                new Stored(
                                        ...
                                        SupportAssistanceKind.valueOf(rs.getString(4)),
                                        rs.getString(5),
                                        rs.getString(6),
                                        rs.getString(7)),
                        supportId,
    ...
    private static SupportAssistanceReceipt receipt(
            Stored row, boolean execute, SupportAssistanceContext.Snapshot input) {

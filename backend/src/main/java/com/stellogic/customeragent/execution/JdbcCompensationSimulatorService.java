...
class JdbcCompensationSimulatorService implements CompensationSimulatorService {
    ...
    @Override
    @Transactional
    public CompensationSimulatorModels.ExecuteResult execute(
        ...
        EffectStatus effect =
                switch (command.scenario()) {
                    ...
                    case BEFORE_EFFECT_FAILURE, RECONCILIATION_NOT_FOUND ->
                            EffectStatus.NOT_OCCURRED;
                    case RECONCILIATION_UNKNOWN -> EffectStatus.UNCERTAIN;
                };
        String reference =
                effect == EffectStatus.SUCCEEDED
                        ? "simulated-refund:" + command.executionId()
                        : null;
        jdbc.update(
    ...
    @Override
    @Transactional
    public CompensationSimulatorModels.ReconciliationResult reconcile(
            UUID executionId, String idempotencyKey) {
        List<ProviderOperation> operations = providerOperation(executionId, true);
        if (operations.isEmpty()) {
            throw new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "provider execution identity not found");
        }
    ...
    private ApprovedExecution approvedExecution(UUID executionId) {
        List<ApprovedExecution> executions =
                jdbc.query(
                        ...
                        (rs, row) ->
                                new ApprovedExecution(
                                        rs.getString(1),
                                        rs.getString(2),
                                        CompensationExecutionModels.CompensationMethod.valueOf(
                                                rs.getString(3)),
                                        rs.getBigDecimal(4)),
                        executionId);
        if (executions.isEmpty()) {
            throw new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "processable execution not found");
        }
        ...
        return jdbc.query(
                ...
                (rs, row) ->
                        new ProviderOperation(
                                ...
                                EffectStatus.valueOf(rs.getString(6)),
                                rs.getString(7),
                                rs.getInt(8)),
                executionId);
    }
    ...
    private record ProviderOperation(
        ...
        CompensationSimulatorModels.ExecuteResult executeResult() {
            CompensationSimulatorModels.ProviderOutcome outcome =
                    switch (effectStatus) {
                        case SUCCEEDED -> CompensationSimulatorModels.ProviderOutcome.SUCCEEDED;
                        case NOT_OCCURRED ->
                                CompensationSimulatorModels.ProviderOutcome.CONFIRMED_NOT_OCCURRED;
                        case UNCERTAIN -> CompensationSimulatorModels.ProviderOutcome.UNKNOWN;
                    };
            boolean responseLost =
                    scenario == CompensationSimulatorModels.Scenario.AFTER_EFFECT_RESPONSE_LOST
                            || scenario
                                    == CompensationSimulatorModels.Scenario.RECONCILIATION_NOT_FOUND
                            || scenario
                                    == CompensationSimulatorModels.Scenario.RECONCILIATION_UNKNOWN;
            return new CompensationSimulatorModels.ExecuteResult(
                    outcome, resultReference, responseLost);
        }
    }
}

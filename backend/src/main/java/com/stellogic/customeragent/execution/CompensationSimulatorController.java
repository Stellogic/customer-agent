...
final class CompensationSimulatorController {
    ...
    CompensationSimulatorController(
            CompensationSimulatorService service, ExecutorMachineIdentity executorIdentity) {
    ...
    CompensationSimulatorModels.ExecuteResult execute(
        ...
        if (idempotencyKey == null
                ...
                || body.amount() == null
                || body.amount().signum() <= 0) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "bound simulator execution required");
        }
        ...
        try {
            ...
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "unsupported simulation scenario");
        }
        var result =
                service.execute(
                        new CompensationSimulatorModels.ExecuteCommand(
                                executionId,
                                idempotencyKey.trim(),
                                body.parameterDigest(),
                                body.amount(),
                                injection));
        if (result.responseLost()) {
            throw new ResponseStatusException(
                    HttpStatus.GATEWAY_TIMEOUT, "simulated provider response lost");
        }
        return result;
    }
    ...
    CompensationSimulatorModels.ReconciliationResult reconcile(
        ...
        if (idempotencyKey == null || idempotencyKey.isBlank() || idempotencyKey.length() > 200) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "original execution identity required");
        }

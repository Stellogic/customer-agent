...
class JdbcClarificationService implements ClarificationService {
    ...
    public ClarificationReplyResult reply(ReplyToClarification command) {
        ...
        String parameterDigest =
                StableParameterDigest.sha256(
                        ...
                        command.clarificationRequestId().toString(),
                        command.resumeRequestId().toString(),
                ...
                jdbc.query(
                        ...
                        (rs, row) ->
                                new ResumeRecord(
                                        rs.getString(1),
                                        rs.getString(2),
                                        AgentResumeStatus.valueOf(rs.getString(3)),
        ...
        if (!existing.isEmpty()) {
            ...
            if (!record.ticketId().equals(command.ticketId())
                    || !record.customerId().equals(command.customerId())) {
                throw new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "clarification resume not found");
            }
                ...
                jdbc.query(
                        ...
                        (rs, row) ->
                                new ReplyScope(
                                        ...
                                        rs.getString(8),
                                        rs.getString(9)),
                        command.clarificationRequestId(),
                        command.ticketId());
        if (scopes.isEmpty() || !scopes.getFirst().customerId().equals(command.customerId())) {
            throw new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "clarification request not found");
        }
        ...
        return new ClarificationReplyResult(
                command.resumeRequestId(), AgentResumeStatus.PENDING, false);
    }

    @Override
    @Transactional(readOnly = true)
    public ClarificationReplyResult status(String customerId, UUID ticketId, UUID resumeRequestId) {
        List<AgentResumeStatus> statuses =
                jdbc.query(
                        ...
                        (rs, row) -> AgentResumeStatus.valueOf(rs.getString(1)),
                        resumeRequestId,
                        ticketId,
                        customerId);
        if (statuses.isEmpty())
    ...
    private void reject(UUID ticketId, HttpStatus status, String reason) {
        audit(
                ticketId,
                "CLARIFICATION_REJECTED_" + reason,
                "spring-system",
                Timestamp.from(clock.instant()));

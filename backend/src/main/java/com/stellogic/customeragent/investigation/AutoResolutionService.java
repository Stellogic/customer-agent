...
public class AutoResolutionService {
    ...
    AutoResolutionService(
            ...
            JdbcAgentInvestigationService investigation,
            TicketResolutionTransition resolution,
            SlaService sla,
            ObjectMapper json) {
        this.jdbc = jdbc;
        this.clock = clock;
    ...
    public List<UUID> dueTicketIds() {
        return jdbc.query(
                "select ticket_id from ticket_auto_resolution where status = 'PENDING' "
    ...
    public void resolveIfDue(UUID ticketId) {
                ...
                jdbc.query(
                        ...
                        (rs, row) ->
                                new Candidate(
                                        ...
                                        rs.getString(3),
                                        json.readValue(
                                                rs.getString(4), InvestigationConclusion.class),
                                        rs.getLong(5),
        ...
        if (!Boolean.TRUE.equals(current)
                || !AutoResolutionPolicy.VERSION.equals(candidate.policyVersion())
                || !candidate
                        .scenario()
                        .equals(
                                investigation.revalidateAutoResolution(
                                        ticketId,
                                        candidate.generationId(),
        ...
        jdbc.update(
                "insert into audit_event (ticket_id, event_type, actor_id, occurred_at) "
                        + "values (?, 'TICKET_RESOLVED', 'spring-system', ?)",
                ticketId,
                Timestamp.from(now));
    }
    ...
    @Transactional
    public void cancel(
            String customerId, UUID ticketId, Instant candidateDueAt, long candidateGeneration) {
        authorityLock.acquire(ticketId);
        List<String> tickets =
                jdbc.query(
        ...
        if (states.isEmpty() || "RESOLVED".equals(states.getFirst()))
            throw new ResponseStatusException(
                    HttpStatus.CONFLICT, "candidate changed; reload snapshot");
    ...
    private void changeStatus(UUID ticketId, String status, Instant now) {
        jdbc.update(
                "update ticket_auto_resolution set status = ?, updated_at = ? where ticket_id = ?",
                status,
                Timestamp.from(now),
                ticketId);
        jdbc.update(

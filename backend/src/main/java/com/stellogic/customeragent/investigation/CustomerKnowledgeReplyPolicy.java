...
final class CustomerKnowledgeReplyPolicy {
    private static final Pattern INSTRUCTION = Pattern.compile(
            "(?i)(忽略.{0,12}(指令|规则|提示)|泄露|系统提示词|developer message|system message|"
                    + "ignore.{0,30}(instruction|rule|prompt)|api[_ -]?key|bearer\\s|<\\|.*?\\|>)");
    ...
    static CustomerKnowledgeProjection validate(
        ...
        if (reply == null || reply.status() == null || reply.answer() == null
                || reply.answer().isBlank() || reply.answer().length() > 1500
                || reply.citations() == null || reply.citations().size() > 5
                || (reply.status() == CustomerKnowledgeStatus.SUPPORTED) != !reply.citations().isEmpty()) {
        ...
        for (CustomerKnowledgeCitation citation : reply.citations()) {
            AgentKnowledgeResult.Source source = receipt.results().stream()
                    .filter(item -> item.articleId().equals(citation.articleId())
                            && item.version().equals(citation.version())
                            && item.chunkId().equals(citation.chunkId()))
                    .findFirst().orElseThrow(() -> invalid("INVALID_KNOWLEDGE_CITATION"));
            if (!source.applicability().contains("CUSTOMER_PUBLIC")
                    || citation.quote() == null || citation.quote().isBlank()
                    || !source.snippet().contains(citation.quote()) || !seen.add(source.chunkId())) {
                throw invalid("INVALID_KNOWLEDGE_CITATION");
            }
            if (INSTRUCTION.matcher(source.snippet()).find()
                    || INSTRUCTION.matcher(source.title()).find()
                    || reply.answer().contains(source.chunkId())
                    || reply.answer().contains(source.articleId())) {
                throw invalid("UNSAFE_KNOWLEDGE");
            }

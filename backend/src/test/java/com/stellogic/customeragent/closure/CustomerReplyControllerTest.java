...
class CustomerReplyControllerTest {
    ...
    private final MockMvc mvc = MockMvcBuilders.standaloneSetup(new CustomerReplyController(service))
    ...
    void customerUsesStableMessageIdentityToReopenTheOriginalTicket() throws Exception {
        ...
        mvc.perform(post("/api/customer/tickets/{ticketId}/replies", ORIGINAL)
    ...
    void differentIssueKindOnTheSameOrderCreatesAnAcceptedLinkedTicket() throws Exception {
        when(service.reply(any())).thenReturn(new CustomerReplyResult(LINKED, "LINKED_TICKET_CREATED", false));

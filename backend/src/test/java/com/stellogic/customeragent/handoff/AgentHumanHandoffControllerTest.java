...
class AgentHumanHandoffControllerTest {
    ...
    private static final UUID GENERATION_ID =
            UUID.fromString("19000000-0000-0000-0000-000000000002");
    private final HumanHandoffService service = org.mockito.Mockito.mock(HumanHandoffService.class);
    private final MockMvc mvc =
            MockMvcBuilders.standaloneSetup(new AgentHumanHandoffController(service, "agent-token"))
                    .build();

    @Test
    void agentRequestsTheSharedAtomicHandoffWithAControlledSummary() throws Exception {
        when(service.requestAgentHumanHandoff(any()))
                .thenReturn(
                        new AgentHumanHandoffResult("handoff-19", "HUMAN", "FACT_CONFLICT", false));

        mvc.perform(
                        post(
                                        "/internal/agent/tickets/{ticketId}/generations/{generationId}/human-handoff",
                                        TICKET_ID,
                                        GENERATION_ID)
    ...
    void rejectsUncontrolledReasonCodesAtTheHttpBoundary() throws Exception {
        mvc.perform(
                        post(
                                ...
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        "{\"reasonCode\":\"MODEL_SAID_SO\",\"summary\":{\"conclusionCode\":\"X\",\"facts\":[]}}"))

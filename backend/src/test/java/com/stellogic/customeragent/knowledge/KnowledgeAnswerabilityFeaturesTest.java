...
class KnowledgeAnswerabilityFeaturesTest {
    ...
    void combinesDenseConfidenceLexicalCoverageAndIndependentRetrieverAgreement() {
        ...
        var actual = KnowledgeAnswerabilityFeatures.extract("alpha beta gamma",
                List.of(first, lexicalOnly), List.of(first, second), List.of(first, second, lexicalOnly));
    ...
    private static KnowledgeRetrievalHit hit(String id, String text, double score) {
        return new KnowledgeRetrievalHit(id, "example", "development-v1", "示例",
                List.of("INTERNAL"), "knowledge/example.md", 1, 1, text, score, null, score);
    }
}

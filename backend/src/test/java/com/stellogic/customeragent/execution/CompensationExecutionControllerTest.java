...
class CompensationExecutionControllerTest {
    ...
    private final MockMvc mvc =
            MockMvcBuilders.standaloneSetup(
                            new CompensationExecutionController(
                                    service, new ExecutorMachineIdentity("executor-secret")))
                    .build();

    @Test
    void assignedExecutorReadsAndClaimsOnlyImmutableExecutionParameters() throws Exception {
        when(service.assignments("compensation-executor"))
                .thenReturn(
                        List.of(
                                new CompensationExecutionModels.Assignment(
                                        EXECUTION_ID,
                                        CompensationExecutionModels.CompensationMethod.COUPON,
                                        new BigDecimal("10.00"),
                                        CompensationExecutionModels.ExecutionStatus.READY,
                                        "compensation-execution:revision")));
        when(service.claim(any()))
                .thenReturn(
                        new CompensationExecutionModels.ClaimResult(
                                EXECUTION_ID,
                                ATTEMPT_ID,
        ...
        mvc.perform(
                        get("/internal/compensation-executions")
        ...
        mvc.perform(
                        post("/internal/compensation-executions/{executionId}/claims", EXECUTION_ID)
                                .header(HttpHeaders.AUTHORIZATION, "Bearer executor-secret")
    ...
    void executorConfirmsSuccessWithoutSupplyingBusinessParameters() throws Exception {
        when(service.succeed(any()))
                .thenReturn(
                        new CompensationExecutionModels.SuccessResult(
                                EXECUTION_ID,
                                ATTEMPT_ID,
        ...
        mvc.perform(
                        post(
                                        "/internal/compensation-executions/{executionId}/success",
                                        EXECUTION_ID)
                                .header(HttpHeaders.AUTHORIZATION, "Bearer executor-secret")
    ...
    @Test
    void executorMarksAmbiguousResultUnknownAndSubmitsAuthoritativeReconciliation()
            throws Exception {
        when(service.markUnknown(any()))
                .thenReturn(
                        new CompensationExecutionModels.TransitionResult(
                                EXECUTION_ID,
                                ATTEMPT_ID,
                                CompensationExecutionModels.ExecutionStatus.UNKNOWN,
                                "补偿结果正在自动确认中，请勿重复提交。",
                                false));
        when(service.reconcile(any()))
                .thenReturn(
                        new CompensationExecutionModels.TransitionResult(
                                EXECUTION_ID,
                                ATTEMPT_ID,
        ...
        mvc.perform(
                        post(
                                        "/internal/compensation-executions/{executionId}/unknown",
                                        EXECUTION_ID)
                                .header(HttpHeaders.AUTHORIZATION, "Bearer executor-secret")
                ...
                .andExpect(jsonPath("$.customerMessage").value("补偿结果正在自动确认中，请勿重复提交。"));

        mvc.perform(
                        post(
                                        "/internal/compensation-executions/{executionId}/reconciliations",
                                        EXECUTION_ID)
                                .header(HttpHeaders.AUTHORIZATION, "Bearer executor-secret")
                                .header(
                                        "Idempotency-Key",
                                        "reconcile:" + EXECUTION_ID + ":provider-query-1")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        """
    ...
    void executorReportsConfirmedPreEffectFailure() throws Exception {
        ...
        mvc.perform(
                        post(
                                        "/internal/compensation-executions/{executionId}/failures",
                                        EXECUTION_ID)
                                .header(HttpHeaders.AUTHORIZATION, "Bearer executor-secret")

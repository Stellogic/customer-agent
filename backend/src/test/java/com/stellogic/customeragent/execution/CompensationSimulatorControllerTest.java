...
class CompensationSimulatorControllerTest {
    ...
    private final CompensationSimulatorService service =
            org.mockito.Mockito.mock(CompensationSimulatorService.class);
    private final MockMvc mvc =
            MockMvcBuilders.standaloneSetup(
                            new CompensationSimulatorController(
    ...
    void simulatorCanLoseTheFirstResponseAfterPersistingTheSideEffect() throws Exception {
        when(service.execute(any()))
                .thenReturn(
                        new CompensationSimulatorModels.ExecuteResult(
        ...
        mvc.perform(
                        post(
                                        "/internal/compensation-simulator/{executionId}/executions",
                                        EXECUTION_ID)
    ...
    void simulatorReconcilesByTheOriginalExecutionIdentity() throws Exception {
        when(service.reconcile(EXECUTION_ID, "compensation-execution:revision"))
                .thenReturn(
                        new CompensationSimulatorModels.ReconciliationResult(
        ...
        mvc.perform(
                        get(
                                        "/internal/compensation-simulator/{executionId}/reconciliation",
                                        EXECUTION_ID)
                                .header(HttpHeaders.AUTHORIZATION, "Bearer executor-secret")
                                .header("Idempotency-Key", "compensation-execution:revision"))

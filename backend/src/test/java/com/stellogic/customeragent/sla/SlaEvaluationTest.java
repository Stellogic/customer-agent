...
class SlaEvaluationTest {
    ...
    void firstResponseUsesContinuousTimeAndEmitsExactBoundaries() {
        assertThat(SlaEvaluation.dueFacts(snapshot(null, 0, CREATED), CREATED.plusSeconds(719)))
                .isEmpty();
        assertThat(
                        SlaEvaluation.dueFacts(
                                snapshot(CREATED.plusSeconds(720), 0, CREATED),
                                CREATED.plusSeconds(900)))
                .containsExactly(
                        new SlaFact(SlaObjective.FIRST_RESPONSE, SlaFactType.WARNING, 720));
        assertThat(
                        SlaEvaluation.dueFacts(
                                snapshot(CREATED.plusSeconds(900), 0, CREATED),
    ...
    void resolutionUsesAccumulatedTimeAndOnlyARunningClockAdvances() {
        ...
        assertThat(
                        SlaEvaluation.dueFacts(
                                snapshot(CREATED, warning - 1, null), CREATED.plusSeconds(100_000)))
                .doesNotContain(
                        new SlaFact(SlaObjective.RESOLUTION, SlaFactType.WARNING, warning - 1));
        assertThat(
                        SlaEvaluation.dueFacts(
                                snapshot(CREATED, warning, null), CREATED.plusSeconds(100_000)))
                .contains(new SlaFact(SlaObjective.RESOLUTION, SlaFactType.WARNING, warning));
        assertThat(
                        SlaEvaluation.dueFacts(
                                snapshot(CREATED, 86_399, CREATED), CREATED.plusSeconds(1)))
    ...
    void resolvedTicketStopsAdvancingButStillMaterializesFactsFromItsFinalElapsedTime() {
        TicketSlaSnapshot resolved =
                new TicketSlaSnapshot(CREATED, CREATED.plusSeconds(900), 90_000, null, "RESOLVED");
    ...
    void reopenedTicketContinuesTheOriginalAccumulatedResolutionBudget() {
        TicketSlaSnapshot reopened =
                new TicketSlaSnapshot(

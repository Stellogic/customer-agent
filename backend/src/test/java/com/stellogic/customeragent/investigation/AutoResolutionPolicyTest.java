...
class AutoResolutionPolicyTest {
    ...
    void springWhitelistAllowsOnlyThreeExplicitScenarioAndConclusionPairs() {
        ...
        assertThat(
                        scenario(
                                ...
                                order(false, false, false, 0, "IN_TRANSIT", 23),
                                "物流状态是什么"))
        ...
        assertThat(
                        scenario(
                                ...
                                order(false, false, false, 0, "IN_TRANSIT"),
                                "请说明订单规则"))
    ...
    void issueKindOrAnAllowedReasonCannotOverrideTheRiskScenario() {
        for (InvestigationRiskScenario risk :
                List.of(
        ...
        assertThat(
                        AutoResolutionPolicy.scenario(
                                conclusion(
                                        DecisionReasonCode.DELAY_UNDER_24_HOURS,
                                        InvestigationRiskScenario.LOGISTICS_DELAY),
                                order(false, false, false, 0, "SIGNED"),
    ...
    void currentPendingActionsCompensationAndLogisticsRiskBlockAnOtherwiseAllowedConclusion() {
        for (var order :
                List.of(
                        order(false, false, false, 1, "IN_TRANSIT"),
                        order(false, true, false, 0, "IN_TRANSIT"),
                        order(false, false, true, 0, "IN_TRANSIT"),
                        order(false, false, false, 0, "SUSPECTED_LOST"),
                        order(false, false, false, 0, "STALLED"))) {
            assertThat(
                            scenario(
                                    DecisionReasonCode.DELAY_UNDER_24_HOURS,
                                    InvestigationRiskScenario.LOGISTICS_DELAY,
    ...
    private static String scenario(
            ...
            InvestigationRiskScenario risk,
    ...
    private static InvestigationConclusion conclusion(
            DecisionReasonCode reason, InvestigationRiskScenario risk) {
    ...
    private static JdbcAgentInvestigationService.ScopedOrder order(
            boolean refunded,
            boolean compensated,
            boolean duplicate,
            int pending,
    ...
    private static JdbcAgentInvestigationService.ScopedOrder order(
            ...
            String logistics,
            int delayHours) {
        return new JdbcAgentInvestigationService.ScopedOrder(
                "ORDER-162",
                delayHours,
                delayHours * 3600L,
                true,

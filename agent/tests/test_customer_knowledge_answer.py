...
def retrieval(scope="CUSTOMER_PUBLIC"):
    return KnowledgeRetrievalResult(7, (KnowledgeSource(
        "signed-package", "v1", "signed-package:1", "签收核实指引", "2026-09-01T00:00:00Z",
        (scope,), 1, 2, TEXT,
    ),))
...
def payload(**citation_changes):
    return {
        "status": "SUPPORTED", "answer": "您可以先检查代收点，并询问同住人员。",
        "citations": [{
            "articleId": "signed-package", "version": "v1", "chunkId": "signed-package:1",
            "quote": TEXT, **citation_changes,
        }],
    }
...
@pytest.mark.parametrize("changes", [{"version": "v0"}, {"articleId": "other"}, {"quote": "已经为您退款"}])
...
def test_insufficiency_is_a_model_output_and_not_inferred_from_an_empty_result():
    answer = parse_customer_knowledge_answer({
        "status": "INSUFFICIENT_INFORMATION", "answer": "现有资料不足以确认，请补充包裹情况。", "citations": [],
    })
    validate_customer_knowledge_citations(answer, retrieval())

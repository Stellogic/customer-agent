...
from baseline_agent.support_assistance_model import generate_support_answer, validate_answer


KNOWLEDGE = {"schema": "agent-knowledge-v1", "indexGeneration": 7, "results": [{
    "articleId": "fixture", "version": "v1", "chunkId": "chunk", "title": "合成政策",
    "updatedAt": "2026-09-01T00:00:00Z", "applicability": ["SUPPORT"],
    "startLine": 1, "endLine": 1,
    "snippet": "这是只用于测试的合成片段，超过二十四个字符，不能作为产品真实知识质量证明。",
}]}
ANSWER = {"decision": "SUPPORTED", "text": "合成测试建议", "followUp": None,
          "citations": [{"chunkId": "chunk", "quote": KNOWLEDGE["results"][0]["snippet"]}]}
REQUEST = {"kind": "draft", "query": "合成问题", "context": {"description": "合成工单"},
           "knowledge": KNOWLEDGE}
...
async def test_one_call_both_decides_and_drafts_and_retains_usage():
    ...
    def respond(request):
        ...
        return httpx.Response(200, json={"id": "fixture-response", "model": "deepseek-v4-flash",
            "status": "completed", "usage": {"input_tokens": 30, "output_tokens": 40, "total_tokens": 70},
...
def test_no_match_may_become_explicit_insufficiency_but_is_not_a_decision_itself():
    answer = {"decision": "INSUFFICIENT_INFORMATION", "text": "现有资料不足，无法确定规则。",
              "followUp": "请补充适用情形。", "citations": []}

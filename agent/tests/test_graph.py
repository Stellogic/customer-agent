...
@pytest.mark.parametrize(
    ...
    [
        ({
            "matchStatus": "UNIQUE", "orderReference": "ORDER-1", "delayHours": 2, "delaySeconds": 1,
            "paid": True, "cancelled": False, "fullyRefunded": False,
            "existingCompensation": False, "pendingActionCount": 0,
            "policyVersion": "delay-policy-v1", "evidenceRefs": ["order:ORDER-1", "logistics:ORDER-1"],
        }, "FACT_CONFLICT"),
        ({"orderReference": ["raw", "payload"], "delayHours": "bad"}, "INVALID_TOOL_RESPONSE"),
        ({
            "matchStatus": "AMBIGUOUS", "orderReference": "ORDER-1", "delayHours": None,
            "delaySeconds": None, "paid": None, "cancelled": None, "fullyRefunded": None,
            "existingCompensation": None, "pendingActionCount": None, "policyVersion": None,
            "evidenceRefs": [], "rawPayload": "must-not-pass",
        }, "INVALID_TOOL_RESPONSE"),
        ({"orderReference": "ORDER-1", "delayHours": 2}, "REQUIRED_FACT_MISSING"),
        ({
            "matchStatus": "UNIQUE", "orderReference": "ORDER-1", "delayHours": 2, "delaySeconds": 7200,
            "paid": False, "cancelled": False, "fullyRefunded": False,
            "existingCompensation": False, "pendingActionCount": 0,
            "policyVersion": "delay-policy-v1", "evidenceRefs": ["order:ORDER-1", "logistics:ORDER-1"],
...
async def test_unsafe_investigation_uses_controlled_handoff_without_leaking_raw_payload(
    monkeypatch: pytest.MonkeyPatch, facts_payload: dict, expected_reason: str,
) -> None:
    ...
    result = await investigate_ticket({
        "requested_by": "spring", "ticket_id": "ticket-19", "generation_id": "generation-19",
    })
...
async def test_conclusion_tool_retry_exhaustion_uses_the_same_stable_handoff_identity(
    ...
    facts = {
        "matchStatus": "UNIQUE", "orderReference": "ORDER-1", "delayHours": 2,
        "delaySeconds": 7200, "paid": True, "cancelled": False, "fullyRefunded": False,
    ...
    assert first["handoff"]["reasonCode"] == replay["handoff"]["reasonCode"] == "TOOL_RETRY_EXHAUSTED"
    assert handoff_keys == [
...
async def test_concurrent_unsafe_tool_results_share_one_handoff_identity(
    ...
    facts = {
        "matchStatus": "UNIQUE", "orderReference": "ORDER-1", "delayHours": 2,
        "delaySeconds": 7200, "paid": False, "cancelled": False, "fullyRefunded": False,
        "existingCompensation": False, "pendingActionCount": 0, "policyVersion": "delay-policy-v1",
        "evidenceRefs": ["order:ORDER-1", "logistics:ORDER-1"],
    }

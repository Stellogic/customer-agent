...
def export_feature_extraction(
    ...
    *,
    ...
    if any(tuple(value.shape) != shape or value.dtype != torch.int64 or value.device.type != "cpu" for value in tokens.values()):
...
class OnnxFeatureExtractor:
    ...
    def extract(self, tokens: dict[str, Any]) -> list[list[float]]:
        ...
        shape = tuple(tokens["input_ids"].shape)
        if len(shape) != 2 or not 1 <= shape[0] <= 32 or not 1 <= shape[1] <= self._protocol.max_seq_length:
            raise ValueError("ONNX 批次/长度超出编码契约")
        if any(tuple(value.shape) != shape or value.dtype != self._numpy.int64 for value in tokens.values()):
            raise ValueError("ONNX 输入须为同形状 int64 数组")
        vectors = self._session.run(["embeddings"], tokens)[0]
        if vectors.shape != (shape[0], self._protocol.output_dimensions) or not self._numpy.isfinite(vectors).all():

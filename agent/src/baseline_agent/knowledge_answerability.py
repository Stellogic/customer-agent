...
QUALITY = {
    ...
    "unanswered_precision": 0.90,
    "unanswered_recall": 0.85,
}
...
def linear_score(features: list[float], policy: dict[str, Any]) -> float:
    ...
    for index in range(4):
        ...
        result += ((features[index] - policy["mean"][index]) / scale) * policy["coefficients"][
            index
        ]
...
def measure(rows: list[dict[str, Any]], decisions: list[bool]) -> dict[str, Any]:
    answered = [
        (row, accepted) for row, accepted in zip(rows, decisions, strict=True) if row["answerable"]
    ]
    unanswered = [
        (row, accepted)
    ...
    return {
        "answered_recall_at_5": sum(recalls) / len(answered),
...
def accepted_rows(rows: list[dict[str, Any]], scores: list[float], threshold: float) -> list[bool]:
    return [
        bool(row["vectorCandidates"]) and score >= threshold
...
def select_threshold(rows: list[dict[str, Any]], scores: list[float]) -> dict[str, Any]:
    ...
    candidates = [
        {"threshold": threshold, "metrics": measure(rows, accepted_rows(rows, scores, threshold))}
    ...
    selected = max(
        feasible,
        key=lambda entry: (
            entry["metrics"]["answered_recall_at_5"],
            entry["metrics"]["unanswered_precision"],
            entry["threshold"],
        ),
    )
...
def fit_once(training: list[dict[str, Any]], calibration: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if any(
        not math.isclose(left, float(right), rel_tol=1e-12, abs_tol=1e-12)
    ...
    result = {
        ...
        "fitted_model": dict(policy),
        "training_label_counts": {
            "supported": sum(labels),
            "unsupported": len(labels) - sum(labels),
        },
        "selection": selection,

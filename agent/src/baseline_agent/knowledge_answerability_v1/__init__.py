...
def load_data(path: Path) -> dict[str, Any]:
    ...
    if data["schema"] != "knowledge-answerability-data-v1" or len(data["topics"]) != expected_topics:
        raise ValueError("独立数据协议或主题数量不符")
    ids = [topic["id"] for topic in data["topics"]]
    if len(set(ids)) != len(ids) or any(not re.fullmatch(r"[a-z][a-z0-9-]{2,30}", key) for key in ids):
    ...
    for topic in data["topics"]:
        ...
        if len(negatives) != 12 or any(sum(row["kind"] == kind for row in negatives) != 6 for kind in ("missing", "mismatch")):
    ...
    for topic in data["topics"]:
        for index, fact in enumerate(topic["facts"], 1):
            for kind in ("direct", "paraphrase"):
                result.append({"id": f"{topic['id']}-{kind}-{index}", "topic": topic["id"],
                    "kind": kind, "text": fact[kind], "answerable": True,
                    "article_id": f"development-{topic['id']}-{fact['article']}",
                    "support": fact["text"], "reason": "指定事实段落完整支持所问内容"})
        for index, negative in enumerate(topic["negatives"], 1):
            result.append({"id": f"{topic['id']}-negative-{index}", "topic": topic["id"],
                "kind": negative["kind"], "text": negative["text"], "answerable": False,
                "article_id": None, "support": None, "reason": negative["reason"]})
    return result

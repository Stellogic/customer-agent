...
def read_seal(path: Path) -> dict[str, Any]:
    seal = read_json(path)
    if (
        seal.get("schema") != "knowledge-answerability-holdout-seal-v1"
        or seal.get("annotationReview") != "PASS"
        or seal.get("topicCount") != 3
        or seal.get("queryCount") != 72
        or seal.get("topicIsolation") is not True
        or seal.get("implementerHasReadContent") is not False
        or not seal.get("authorContext")
        or not seal.get("reviewerContext")
        or seal["authorContext"] == seal["reviewerContext"]
        or len(seal.get("datasetSha256", "")) != 64
        or not seal.get("sealedAt")
    ):
...
def applied_holdout_policy(fit_report: Path, seal: Path) -> dict[str, Any]:
    ...
    active_path = (
        ROOT.parents[3] / "backend/src/main/resources/knowledge-answerability-logistic.json"
    )
...
def collect(data: dict[str, Any], base_url: str, report: dict[str, Any]) -> None:
    ...
    with httpx.Client(base_url=base_url, timeout=120) as client:
        ...
        if (
            state["status"] != "READY"
            or state["failureCode"] is not None
            or state["articleCount"] != len(expected)
        ):
        ...
        for article_id, source in expected.items():
            ...
            if (
                ...
                or detail["body"] != source["body"]
                or detail["title"] != source["title"]
                or detail["applicability"] != ["INTERNAL"]
                or detail["publicationStatus"] != "PUBLISHED"
                or not detail["current"]
        ...
        for query in queries(data):
            response = client.get(
                "/api/internal/knowledge/development-candidates",
                params={"q": query["text"], "scope": "INTERNAL"},
            )
            response.raise_for_status()
            candidate = response.json()
            if (
                candidate["schema"] != "knowledge-development-v1"
            ...
            for key, limit in (
                ...
                for hit in candidate[key]:
                    ...
                    if (
                        known is None
                        or hit["articleId"] != known["articleId"]
                        or hit["version"] != VERSION
                        or hit["applicability"] != ["INTERNAL"]
            ...
            expected_chunks = [
                ...
                if query["answerable"]
                and chunk["articleId"] == query["article_id"]
            ...
            row = {
                ...
                **candidate,
                ...
                "recall": len(set(hits) & set(expected_chunks)) / len(expected_chunks)
                if expected_chunks
                else 0.0,
...
def checked_observations(path: Path, split: str, head: str | None = None) -> dict[str, Any]:
    ...
    if (
        ...
        or (head and report["head_sha"] != head)
    ):
...
def fit_reports(args: argparse.Namespace, report: dict[str, Any]) -> None:
    ...
    for observed in (training, calibration):
        expected = next(
            entry for entry in manifest["datasets"] if entry["split"] == observed["split"]
        )
        if (
            expected["annotationReview"] != "PASS"
            or observed["dataset_sha256"] != expected["sha256"]
    ...
    if set(row["topic"] for row in training["rows"]) & set(
        row["topic"] for row in calibration["rows"]
    ):
    ...
    if safety.get("head_sha") != args.head_sha or any(
        safety.get("checks", {}).get(key) != "PASS" for key in required
    ):
        raise ValueError("缺少当前源码的独立权限/版本/范围/数值回归证据")
    if version("scikit-learn") != "1.7.2":
        raise ValueError("训练器版本与预定依赖不符")
    report["environment"].update(
        {name: version(name) for name in ("scikit-learn", "numpy", "scipy")}
    )
    report["inputs"] = {
        "training": file_sha(args.training_report),
        "calibration": file_sha(args.calibration_report),
    ...
    if policy is not None:
        policy.update(
            modelRevision=report["model_revision"],
            sourceSha=args.head_sha,
            trainingDatasetSha256=training["dataset_sha256"],
            calibrationDatasetSha256=calibration["dataset_sha256"],
            holdoutSealSha256=file_sha(args.holdout_seal),
            dependencyVersions={"scikit-learn": version("scikit-learn")},
        )
...
def audit_report(args: argparse.Namespace, report: dict[str, Any]) -> None:
    ...
    if observed["dataset_sha256"] != report["holdout_seal"]["datasetSha256"] or policy[
        "holdoutSealSha256"
    ] != file_sha(args.holdout_seal):
    ...
    for row, accepted in zip(rows, decisions, strict=True):
        ...
        if (
            actual != expected
            or product["policy"]["id"] != policy["id"]
    ...
    report.update(
        status="PASS" if all(values[key] >= target for key, target in QUALITY.items()) else "FAIL",
        metrics=values,
        scores=scores,
        observations_sha256=file_sha(args.observations),
        fit_report_sha256=file_sha(args.fit_report),
...
def main() -> None:
    ...
    with args.output.open("x", encoding="utf-8", newline="\n") as output:
        ...
        report: dict[str, Any] = {
            "schema": "knowledge-answerability-run-v1",
            "status": "ERROR",
            "phase": args.phase,
            "run_id": args.run_id,
        ...
        try:
            ...
            if args.phase in ("prepare", "collect"):
                ...
                if args.split == "holdout":
                    ...
                    if (
                        entry["annotationReview"] != "PASS"
                        or file_sha(args.dataset) != entry["sha256"]
                    ):
